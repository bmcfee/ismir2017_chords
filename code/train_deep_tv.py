...
class TVnorm(K.regularizers.Regularizer):
    '''Total-variation regularization

    # Arguments:
        l : float; regularization factor.
        axis : int; axis along which differences are computed.
    '''

    def __init__(self, l=0.01, axis=1):
        self.l = K.backend.cast_to_floatx(l)
        self.axis = axis

    def get_config(self):
        return {'l': float(self.l), 'axis': self.axis}

    def __call__(self, x):
        idx1 = [Ellipsis] * x.ndim
        idx2 = [Ellipsis] * x.ndim
        idx1[self.axis] = slice(1, None)
        idx2[self.axis] = slice(x.shape[self.axis] - 1)
