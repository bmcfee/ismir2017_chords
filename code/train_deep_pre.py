...
def construct_model(pump, structured):
    ...
    if structured:
        ...
        # 4: merge layer
        codec_1 = K.layers.concatenate([pc_p, root_p, bass_p])

        p0 = K.layers.Dense(len(pump['chord_tag'].vocabulary()),
                            activation='linear', use_bias=False)

        tag_0 = K.layers.TimeDistributed(p0)(codec_1)

        weights = np.load('pre-trained-decoder.npy').T
        p0.set_weights([weights])

        p1 = K.layers.Dense(len(pump['chord_tag'].vocabulary()),
                            activation='linear',
                            bias_regularizer=K.regularizers.l2())

        tag_1 = K.layers.TimeDistributed(p1)(rnn)
        tag = K.layers.Activation('softmax', name='chord_tag')(K.layers.add([tag_0, tag_1]))
