...
In our experiments, we evaluate the following decoder architectures:
\begin{description}
...
%, and yields the best-performing model (\emph{CR2+S+A}).
...
In \Cref{fig:confusion},the large values along the main diagonal indicate general high accuracy at predicting the exact (simplified) chord label (63.6\%).
We observe that the model's most common mistakes are related to sevenths, \eg, \texttt{7}$\rightarrow$\texttt{maj}, \texttt{dim7}$\rightarrow$\texttt{maj7}, and \texttt{hdim7}$\rightarrow$\texttt{dim} or \texttt{min6}, though it appears to not confuse different seventh chords with each-other.

The low accuracy of predicting out-of-gamut \texttt{X}-chords (bottom row) indicates that the model is reluctant to predict that symbol.
In the entire corpus, there are 4557 instances (about 3\%) which map to \texttt{X}: 2091 of which are 1-chords (\eg, \texttt{C:1}), 2365 of which are power chords (\eg, \texttt{C:5}), and 101 \texttt{X}-chords.
The model appears to distribute these
% TODO: find something to say about this
...
For comparison purposes, the difference in the confusion matrices between \emph{CR2+S+A} and the unstructured model \emph{CR2+A} is depicted in \Cref{fig:confdelta}.
...
While the net effect is positive, increasing accuracy by +0.8\%, the structured model slightly degrades accuracy for the rarest qualities \texttt{dim7} and \texttt{minmaj7}.
This is compensated for by the increase in accuracy for \texttt{7} (reducing confusions with \texttt{maj}) and \texttt{maj6} (reducing coonfusions with \texttt{hdim7}, \texttt{maj}, and \texttt{minmaj7}).
Note however that the overall accuracy remains high, as these classes still have large diagonal values in \Cref{fig:confusion}.
...
However, doing so effectively---and evaluating the resulting predictions---would require larger annotated corpora for these classes than we currently have access to.
