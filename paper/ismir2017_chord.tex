...
%\end{description}
We investigate two models for decoding $h(t)$ to the sequence of chord labels $\hat{y}(t)$, depicted in \Cref{fig:architectures}.
...
Minimizing the \emph{root} and \emph{pitches} losses corresponds to (approximately) maximizing the \emph{root} and \emph{tetrads} recall scores during training, while \Cref{objective} learns the decoding into the human-readable chord vocabulary.
