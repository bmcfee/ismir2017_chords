...
\begin{align*}
    ...
    \end{cases}.
\end{align*}
The set of active pitch classes is matched against a set of 14 templates: \texttt{maj, min, dim, aug, min6, maj6, min7, maj7, dom7, dim7, hdim7, minmaj7, sus2, sus4}.
...
% TODO:
%   rare classes look like noise
%   structure can help exploit commonalities
...
%These effects combine to inhibit the model's ability to capture rare classes, where scarcity of observations make it difficult to .
...
% catch-all class can crowd the space
% but maybe you'd still get some points at prediction time by predicting a nearby chord



%The structured representation preserves the pitch content of these classes, allowing the model to 
...
% Note: in general, it's not trivial to invert root+chroma -> chord, so we still learn this as part of the decoder
...
This is to be expected, since the CR models do not model quality independently of root.
...
Second, structured training (\emph{+S} variants) generally provides a modest, but consistent improvement, and yields the best-performing model (\emph{CR2+S+A}).
...
Third, the deep model \emph{CR2+A} performs comparably or slightly better than the wide model \emph{CR1+A}.
The gaps are more pronounced for the structured variants.
...
\begin{figure}
    ...
    Results are aggregated across frames in all test tracks, and compare the frame's predicted label to its reference label in the simplified vocabulary.\label{fig:confusion}}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{confdelta}
    \caption{The difference between confusion matrices for \emph{CR2+S+A} and the unstructured \emph{CR2+A}.
             Positive values along the diagonal indicate increased accuracy.\label{fig:confdelta}}
\end{figure}
